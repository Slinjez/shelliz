...
class JWT {
    ...
    public static function decode($token, $key, array $allowed_algs = array())
    {
        ...
        if (isset($payload->exp)){
            $timeexp = date_create_from_format('Y-m-d H:i:s', $payload->exp);
            $expiry = $timeexp->getTimestamp();
            $payload->exp = $expiry;
        }
        if (isset($payload->nbf)){
            $timenbf = date_create_from_format('Y-m-d H:i:s', $payload->nbf);
            $bestBefore = $timenbf->getTimestamp();
            $payload->nbf = $bestBefore;
        }
        if (isset($payload->nbf) && $payload->nbf > ($timestamp + static::$leeway)) {
            throw new Exception(
                'Cannot handle token before to ' . date(DateTime::ISO8601, $payload->nbf)
            );
        }
        ...
        if (isset($payload->exp) && ($timestamp - static::$leeway) >= $expiry) {
            $payload->nbf = ''; 
            $payload->exp = '';
            $payload->sts = 'expired token';
        }
        return $payload;
    }
